...
import { ExportDropdown } from "@/components/invoice/ExportDropdown";
import { formatCurrency, formatDate, formatGSTIN } from "@/lib/utils";
import type { Invoice, InvoiceStatus, SubscriptionPlan } from "@/types";
...
function StatusBadge({ status }: { status: InvoiceStatus }) {
  ...
  return (
    <span
      className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide ${cfg.classes}`}
    >
      {cfg.label}
    </span>
  );
...
function ExpandedDetail({ invoice }: { invoice: Invoice }) {
  const lineItems = Array.isArray(invoice.raw_ai_response?.line_items)
...
export function InvoiceHistoryClient({ userPlan }: InvoiceHistoryClientProps) {
    ...
    <div className="max-w-5xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <ExportDropdown userPlan={userPlan} />
          <button
        ...
        <AnimatePresence>
          {showFilters && (
            ...
            >
              ...
              <div className="overflow-x-auto scroll-x-touch -mx-1 px-1 pb-1">
                <div className="flex sm:grid sm:grid-cols-3 gap-3 min-w-[480px] sm:min-w-0">
                  <div>
