...
type UploadState =
  ...
  | { step: "parsed"; data: ParsedInvoiceData; fileUrl: string; file: File }
  | { step: "saving"; data: ParsedInvoiceData; fileUrl: string; file: File }
  | { step: "saved" };
...
export default function InvoiceUploadPage() {
  ...
  const handleSave = async () => {
    ...
    const { data, fileUrl, file } = state;
    const parsedState = { data, fileUrl, file };
    setState({ step: "saving", data, fileUrl, file });
    ...
    <div className="max-w-3xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Upload any invoice — our AI extracts all GST data automatically
        </p>
      </div>

      {/* ── Step indicator (visible when past idle) ── */}
      <AnimatePresence>
        {currentStep !== "idle" && (
          <motion.div
      ...
      <div className="mb-6">
        <UploadZone
          ...
          uploadProgress={
            state.step === "uploading" ? state.progress : 0
          }
          selectedFile={
      ...
      <AnimatePresence>
        {(currentStep === "parsed" || currentStep === "saving") && (
          <motion.div
          ...
          >
            <ParseResult
              data={(state as Extract<UploadState, { step: "parsed" | "saving" }>).data}
              fileName={(state as Extract<UploadState, { step: "parsed" | "saving" }>).file.name}
            />

            <div className="flex items-center justify-between gap-3 mt-6 pt-4" style={{ borderTop: "1px solid hsl(var(--border))" }}>
              <button
                onClick={handleReset}
          ...
          {[
            ...
            >
              <div className="w-9 h-9 rounded-xl bg-saffron-100 flex items-center justify-center mx-auto mb-3">
                <Icon className="h-4.5 w-4.5 text-saffron-600" style={{ width: "18px", height: "18px" }} />
              </div>
